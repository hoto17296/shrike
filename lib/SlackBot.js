...
class SlackBot extends EventEmitter {
  ...
  send(msg) {
    return new Promise((resolve, reject) => {
      ...
      if ( msg.channel_id ) {
        msg.channel = msg.channel_id;
        delete msg.channel_id;
      }
      else if ( msg.channel ) {
        let channel = this.data.channels.filter((channel) => channel.name === msg.channel)[0];
        if ( ! channel ) return reject('Channel "' + msg.channel + '" not found.');
        msg.channel = channel.id;
      }
      else {
        msg.channel = this.default_channel.id;
      }
